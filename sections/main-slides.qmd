...
::: {.boxed}
Always **save your functions in** the folder **`R/`**[^rfolder]
:::


[^anonym]: In {{< fa brands r-project >}}&nbsp; there is also **anonymous functions** (also known as **lambda functions**):<br />`function(x) sum(x) / length(x)`<br />or in short: `\(x) sum(x) / length(x)`

[^rfolder]: The folder **`R/`** must contain **only functions** (no script)
...
{{< fa circle-arrow-right >}}&nbsp; Do not forget to **source the file each time** you modify the function
...
{{< fa circle-arrow-right >}}&nbsp; Object created in the **`global environment`** **is accessible** from the function
...
{{< fa circle-arrow-right >}}&nbsp; **Easy to use** now!
